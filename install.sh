...
skills_hint() {
  ...
  if [ -n "$url" ]; then
    ...
    echo "    /plugin install <plugin>@<marketplace-name>   # e.g. pce-troubleshooting@skills-bundler"
  else
    echo "  apex-router ships no skills. To share team skills (PCE/VEN ops, workflows) via a PRIVATE"
